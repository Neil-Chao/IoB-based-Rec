...
from GPR.model.BASE.BASE import BASE
...
def train(model, dataloader, loss_fn, optimizer, logging, max_iter=100, lr=0.0001, lambda2=0.1, conv=1000):
    checkpoint_path = os.path.join(output_dir, "GPR " + datetime.now(ZoneInfo('Asia/Shanghai')).strftime("%Y-%m-%d %H_%M_%S")+".pt")
    ...
    for iter in range(max_iter):
        ...
        recall, mrr, ndcg = evaluate(model, val_dataloader, model.item_num, GPR_CONFIG.RECOMMEND_NUM)
...
if __name__ == "__main__":
    ...
    import logging
    path = os.path.join("./GPR/logs/GPR", datetime.now(ZoneInfo('Asia/Shanghai')).strftime("%Y-%m-%d %H_%M_%S")+".log")
    ...
    model = BASE(user_num, item_num, GPR_CONFIG.USER_EMB_SIZE, GPR_CONFIG.ITEM_EMB_SIZE).cuda()
