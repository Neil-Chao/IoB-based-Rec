...
import GPR.config.SMGPR as config
import torch
import numpy as np
from torch.utils.data import DataLoader, SubsetRandomSampler
from GPR.model.TorchDataset.GroupDatasetWithUserHistory import UnloadedGroupDatasetWithUserHistory, UnloadedGroupDatasetWithNegative
from GPR.model.SMGPR.SMGPR import SMGPR
import math
from pytz import timezone as ZoneInfo
from GPR.utils.EarlyStopping import EarlyStopping

output_dir = r'oss/saved_model/' + config.SCENARIO
...
def loss_fn(y_hat, pos_y):
    p_score = torch.gather(y_hat, 1, pos_y.unsqueeze(1)).squeeze()
    return torch.mean(-torch.log(p_score + 1e-16))
...
def train(model, train_dataloader, val_dataloader, test_dataloader, loss_fn, optimizer):
    checkpoint_path = os.path.join(output_dir, "SMGPR " + datetime.now(ZoneInfo('Asia/Shanghai')).strftime("%Y-%m-%d %H_%M_%S")+".pt")
    es = EarlyStopping(checkpoint_path, patience=16, verbose=True)
    ...
    for iter in range(config.MAX_ITER):
        ...
        for i, (g, gg, X, u, positive, negative) in enumerate(train_dataloader):
            ...
            tmp_loss = loss_fn(p_hat, positive)
            loss += tmp_loss
            optimal_obj = tmp_loss
            optimizer.zero_grad()
            optimal_obj.backward()
...
if __name__ == "__main__":
    ...
    train_dataloader, val_dataloader, test_dataloader, user_num, item_num = get_dataloader(SCENARIO = config.SCENARIO)

    path = os.path.join("./GPR/logs/SMGPR", datetime.now(ZoneInfo('Asia/Shanghai')).strftime("%Y-%m-%d %H_%M_%S")+".log")
    ...
    model = SMGPR(user_num, item_num+1, user_emb_size=32, item_emb_size=32, alpha=config.ALPHA, pooling=config.POOLING, similarity_vec=similarity_vec, factor=config.FACTOR).cuda()
