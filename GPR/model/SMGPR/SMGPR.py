...
from GPR.model.SMGPR.bert_modules.bert import BERT
from GPR.model.SMGPR.bert_modules.utils.outlayer import OutLayer

from GPR.model.SMGPR.GroupEmbedding import GroupEmbedding
...
class SMGPR(nn.Module):
    ...
    '''
    def __init__(self, user_num, item_num, user_emb_size, item_emb_size, similarity_vec, bert_max_len=6, bert_num_blocks=2, bert_num_heads=4, bert_dropout=0.1, alpha=1e-3, pooling="sum", factor=0.5) -> None:
        super().__init__()
        fix_random_seed_as(0)
        self.group_layer = GroupEmbedding(user_num+1, item_num+1, user_emb_size, item_emb_size, pooling, factor)
        ...
        self.memory_model = BERT(bert_max_len, item_num, bert_num_blocks, bert_num_heads, hidden_size, bert_dropout)
        self.alpha = alpha
        self.similarity_vec = similarity_vec

        self.out = OutLayer(hidden_size, user_num)

    def forward(self, group_user, group_behavior, x, target_user, train=False):

        group_embedding = self.group_layer(group_user, group_behavior, self.memory_model.embedding.token, target_user, self.similarity_vec)
