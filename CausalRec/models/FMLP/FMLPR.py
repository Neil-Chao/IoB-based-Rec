...
class SelfAttention(nn.Module):
    def __init__(self, hidden_size, num_attention_heads, attention_probs_dropout_prob):
        ...
        self.out_dropout = nn.Dropout(attention_probs_dropout_prob)
...
class CausalLayer(nn.Module):
    ...
    def forward(self, bert_x, x, target):
        res = bert_x.clone()
        for i in range(bert_x.size(1) - 1):
            source = x[:, i]
            fac = (self.ae[source].unsqueeze(1) @ self.w @ self.ae[target].unsqueeze(2)).squeeze()
            res[:, -1] += bert_x[:, i] * (( (i+1) / bert_x.size(1)) * fac).view(-1, 1)
        return res

class FMLPR(nn.Module):
    def __init__(self, item_size, hidden_size=64, max_seq_length=5, hidden_dropout_prob=0.5, initializer_range=0.02,  num_hidden_layers=2, no_filters=False, num_attention_heads=2, attention_probs_dropout_prob=0.5, hidden_act="gelu", t=0.3, gamma1=5, gamma2=5):
        super(FMLPR, self).__init__()
        cluster = 16
        self.gamma1 = gamma1
        self.gamma2 = gamma2
        self.ae = nn.Parameter(torch.zeros(item_size, cluster).to(torch.float32), requires_grad=True)
        self.w = nn.Parameter(torch.zeros(cluster, cluster).to(torch.float32), requires_grad=True)
    ...
    def forward(self, input_ids, y):
        ...
        x = self.cl(item_encoded_layers[-1], input_ids, y)
        sequence_output = x
